...
public class ForgeEventHelper implements IPlatformEventHelper {
    ...
    private void registerListener(Object target, Method method) {
        ...
        Class<? extends EventWrapper> eventClass = (Class<? extends EventWrapper>) params[0];

        Class<? extends Event> forgeEventClass = IPlatformEventHelper.getForgeEventClass(eventClass);
        ...
        MinecraftForge.EVENT_BUS.addListener(priority, receiveCanceled, forgeEventClass, event -> {
            try {
                Constructor<?> ctor = eventClass.getConstructor(forgeEventClass);
